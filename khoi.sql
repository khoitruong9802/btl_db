...
--
-- Host: 127.0.0.1
-- Generation Time: Dec 10, 2023 at 07:59 AM
...
-- Procedures
--
CREATE DEFINER=`root`@`localhost` PROCEDURE `total_service_use` (IN `begin_date` DATE, IN `end_date` DATE)   BEGIN
...
-- Functions
--
CREATE DEFINER=`root`@`localhost` FUNCTION `room_payment` (`room_rent_id` INT) RETURNS INT(11)  BEGIN
    SELECT `book_id` FROM roomrentinfo WHERE id = room_rent_id INTO @room_book_id;
    SELECT SUM(rt.price) FROM assignroom AS a
    INNER JOIN room AS r ON a.room_id = r.id 
    INNER JOIN roomtype AS rt ON r.roomtype = rt.id 
    WHERE a.order_id = @room_book_id
    INTO @room_cost;
    SELECT SUM(s.cost * su.numberofservice) FROM `serviceuse` AS su
    INNER JOIN `serviceuseinfo` AS sui ON su.serviceuse_id = sui.id
    INNER JOIN `roomrentinfo` AS rri ON sui.rent_id = rri.id
    INNER JOIN `service` AS s ON su.service_id = s.id
    WHERE rri.id = 1
    INTO @service_cost;
    SET @total_cost = @room_cost + @service_cost;
    RETURN @total_cost;
END$$
...
INSERT INTO `discount_code` (`id`, `salepercent`, `maxsale`, `expiredate`) VALUES
(1, 20, 100000, '2023-12-31'),
(2, 30, 200000, '2023-12-31'),
(3, 15, 100000, '2023-12-31'),
(4, 40, 300000, '2023-12-31'),
(8, 80, 30000, '2023-12-27');
...
(21, 1, '2023-12-29', '2023-12-30', '2023-12-09', 1, 1, 6, 1),
(22, 0, '2023-12-20', '2023-12-21', '2023-12-09', 2, 3, 7, 1);
...
(6, '2023-12-09', 2, 0, 13, 1),
(7, '2023-12-09', 2, 0, 18, 1),
(8, '2023-12-09', 4, 4, 21, 1);
...
--
ALTER TABLE `bill`
  ADD CONSTRAINT `bill_ibfk_1` FOREIGN KEY (`rentid`) REFERENCES `roomrentinfo` (`id`),
  ADD CONSTRAINT `bill_ibfk_2` FOREIGN KEY (`discountcode`) REFERENCES `discount_code` (`id`),
  ADD CONSTRAINT `bill_ibfk_3` FOREIGN KEY (`paymethod_id`) REFERENCES `pay_method` (`id`);
